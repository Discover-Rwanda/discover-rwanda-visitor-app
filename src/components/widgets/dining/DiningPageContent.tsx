...
interface DiningPageContentProps {
  dining: Dining[];
  total: number;
  currentPage: number;
  pageSize: number;
...
const DiningPageContent: React.FC<DiningPageContentProps> = ({
  dining,
  total,
  currentPage,
  pageSize,
  ...
  const uniqueLocations = [...new Set(dining.map(item => item.location))];
  const categories = [
  ...
  return (
    <div className='bg-white'>
      <HeroSection
        title="Discover Rwanda's Dining"
        description="Explore the best places to eat in Rwanda, from fine dining to casual cafes."
        searchPlaceholder="Search dining..."
        backgroundImageUrl="/images/Local-cuisine.jpg"
        searchValue={search}
          ...
          <div className="lg:hidden flex justify-between items-center mb-4">
            <div>
              <p className="text-gray-700">
                Showing {total} {total === 1 ? 'place' : 'places'}
              </p>
            </div>
            <Button
              variant="outline"
...
export default DiningPageContent; 