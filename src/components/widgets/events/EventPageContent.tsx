...
const EventPageContent: React.FC<EventPageContentProps> = ({
  events,
  total,
  currentPage,
  pageSize,
  ...
  const uniqueLocations = [...new Set(events.map(item => item.location))];
  const categories = [
  ...
  const handleMonthChange = (month: string) => {
    updateParams({ month, page: '1' });
  };
  const handlePageChange = (page: number) => {
    updateParams({ page: String(page) });
  };
  const handleResetFilters = () => {
    updateParams({ search: '', category: [], location: [], month: 'all', page: '1' });
  };
  ...
  return (
    <div className='bg-white'>
      <HeroSection
        title="Discover Rwanda's Events"
        description="Explore upcoming events, festivals, and cultural celebrations across Rwanda."
        searchPlaceholder="Search events..."
        backgroundImageUrl="/images/Umuganura-Muhondo-Gakenke-Paying-tribute-to-the-king.jpg"
        searchValue={search}
          ...
          <div className="lg:w-1/4 hidden lg:block">
            <div className="bg-white rounded-lg border border-gray-200 shadow-sm p-6 sticky top-32">
              <div className="flex justify-between items-center mb-6">
                ...
                {(selectedCategories.length > 0 || selectedLocations.length > 0 || selectedMonth !== 'all') && (
                  <Button
                    variant="ghost"
              ...
              <div className="space-y-6">
                ...
                <div>
                  <h3 className="font-medium mb-3 text-black">Month</h3>
                  <select
